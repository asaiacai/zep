...
from sky.adaptors import jobset
from sky.clouds import Jobset
...
_image_df = common.read_catalog('jobset/images.csv',
...
def list_accelerators_realtime(
    ...
    k8s_jobset_cloud = Jobset()
    if not any(
            map(k8s_jobset_cloud.is_same_cloud,
    ...
    for accelerator_name, accelerator_count in accelerators_qtys:
        result.append(
            common.InstanceTypeInfo(cloud='Jobset',
                                    instance_type=None,
    ...
    qtys_map = common.list_accelerators_impl('Jobset', df, gpus_only,
